...
def extract(full_command, args):
    ...
    if skip_extraction:
        ...
        if dna_ref["DNA"]["NT_path"]:
            ...
            log.log(f"{'dna_depth_tolerance':>{mar}}: {dna_dt_msg}")
        log.log("")
        log.log("")
    ...
    if skip_clustering:
        ...
        log.log(f"{'gap_open':>{mar}}: {bold(settings.MMSEQS_GAP_OPEN_NT)}")
        log.log(f"{'gap_extend':>{mar}}: {bold(settings.MMSEQS_GAP_EXTEND_NT)}")
        ...
        if dir_is_empty(clustering_dir) or clustering_input_file.is_file() or args.overwrite:
            ...
            if clust_ref["CLR"]["NT_path"]:
                ...
                log.log(f"{'dna_depth_tolerance':>{mar}}: {clr_dt_msg}")
            log.log("")
            log.log("")
